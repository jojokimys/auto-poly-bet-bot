...
export function MMLogFeed() {
  const mmLogs = useMMStore((s) => s.logs);
  const sniperLogs = useMMStore((s) => s.sniperLogs);
  // Show sniper logs if available, otherwise MM logs
  const logs = sniperLogs.length > 0 ? sniperLogs : mmLogs;
    ...
    <Card>
      <CardBody className="py-3">
        ...
        >
          ...
          {sniperLogs.length > 0 ? 'Sniper' : 'MM'} Activity Log
